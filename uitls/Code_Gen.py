...
import requests
from uitls.feed import readFeedAtom, readFeedHTML, readFeedJSON, readFeedRSS
import os
...
def scan_for(value, soup, tab, isTime, listP=[]):
    ...
    if len(soup.select('[itemprop="{value}"]'.format(value=value))) != 0:
        found = False
        array.append(
            ("\t"*(tab+0)) + "{value} = soup.select('[itemprop=\"{value}\"]')[0][content].get_text()".format(value=value))
    ...
    ##
        ...
        found = False
        array.append(
            ("\t"*(tab+0)) + "{value} = soup.select('[property=\"{value}\"]')[0][content].get_text()".format(value=value))
        ...
        found = False
        array.append(
            ("\t"*(tab+0)) + "i = soup.select('[property=\"{value}\"]')[0][content].get_text()".format(value=value))
    elif len(soup.select('.{value}'.format(value=value))) != 0:
        found = False
        array.append(
            ("\t"*(tab+0)) + "{value} = soup.select('.{value}').get_text()".format(value=value))
    return array, found


class CodeGen:
    def __init__(self, thing) -> None:
        if "official_website" in thing.keys():
            self.url = thing["official_website"]
        self.url = []
        self.list_pace = []
        if "web_feed_URL" in thing.keys():
            self.feed = thing["web_feed_URL"]
        self.feed = []
        self.name = ""
    ...
    def gen_article_conditionsOfAccess(self, soup, tab):
        # string
        array, found = scan_for("conditionsOfAccess",
                                soup, tab, True, listP=[])
        if found:
            return array
        else:
    ...
    def gen_article_description(self, soup, tab):
        ...
        if found:
            ...
            array.append(
                ("\t"*tab)+"description = soup.select('#content-wrapper > h2')[0].get_text()")
        elif len(soup.select('.metabox > h4')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.metabox > h4')[0].get_text()")
        elif len(soup.select('.body-excerpt > p')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.body-excerpt > p')[0].get_text()")
        elif len(soup.select('h2.polaris__heading')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('h2.polaris__heading')[0].get_text()")
        elif len(soup.select('h2.c-header__intro ')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('h2.c-header__intro ')[0].get_text()")
        elif len(soup.select('.post-full-custom-excerpt')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.post-full-custom-excerpt')[0].get_text()")
        elif len(soup.select('div > div > div.col-8.single-maincol > p.intropara')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('div > div > div.col-8.single-maincol > p.intropara')[0].get_text()")
        elif len(soup.select('#content-wrapper > h2')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('#content-wrapper > h2')[0].get_text()")
        elif len(soup.select('.td-post-content')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.td-post-content')[0].get_text()")
        elif len(soup.select('.newspack-post-subtitle')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.newspack-post-subtitle'))[0[0].get_text()")
        elif len(soup.select('.quote')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.quote')[0].get_text()")
        elif len(soup.select('header.details-header > h4')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('header.details-header > h4')[0].get_text()")
        elif len(soup.select('.c-article-excerpt')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.c-article-excerpt')[0].get_text()")
        elif len(soup.select('.sdc-article-header__sub-title')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.sdc-article-header__sub-title')[0].get_text()")
        elif len(soup.select('#main > div > h2')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('#main > div > h2')[0].get_text()")
        elif len(soup.select('p.hdr-description')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('p.hdr-description')[0].get_text()")
        elif len(soup.select('.header > hgroup > h2')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.header > hgroup > h2')[0].get_text()")
        elif len(soup.select('.strapline')) != 0:
            array.append(
                ("\t"*tab)+"description = soup.select('.strapline')[0].get_text()")
        else:
            return []
    ...
    def gen_article_isAccessibleForFree(self, soup, tab):
        array, found = scan_for("isAccessibleForFree",
                                soup, tab, True, listP=[])
        if found:
            return array
        else:
    ...
    def gen_article_associatedMediaReview(self, soup, tab):
        array, found = scan_for("associatedMediaReview",
                                soup, tab, True, listP=[])
        if found:
            return array
        else:
    ...
    def gen_article_associatedClaimReview(self, soup, tab):
        array, found = scan_for("associatedClaimReview",
                                soup, tab, True, listP=[])
        if found:
            return array
        else:
    ...
    async def gen_article_article(self):
        seen = []
        code = []
        for url in self.list_pace:
            print("GEN:", url)
            array = []
            r = await reqest_saferobot(url)
    ...
    async def getURl(self):
        for i_url in self.feed:
            ...
            if isGood:
                ...
                continue
            isGood,  list_pace = readFeedRSS(response.content)
            if isGood:
                self.list_pace = self.list_pace + list_pace
                continue
            isGood,  list_pace = readFeedAtom(response.content)
            if isGood:
                self.list_pace = self.list_pace + list_pace
                continue
            isGood,  list_pace = readFeedJSON(response.content)
            if isGood:
                self.list_pace = self.list_pace + list_pace
                continue
    ...
    def gen(self):
        ...
        p = os.path.join("./", "websites", self.name+".py")
        f = open(p, "w")
        f.write(dText)
        f.close()
