...
feeds = [
    "https://www.birminghammail.co.uk/rss.xml"
]
header={
...
def article(url):
    ...
    for s in soup.select('[itemprop="articleBody"] > p:contains("READ MORE:")'):
        s.extract()
    for s in soup.select('[itemprop="articleBody"] > p:contains("To get the latest updates on crime and court cases straight to your inbox click")'):
        s.extract()
    for s in soup.select('script'):
        s.extract()
    article =  soup.select('[itemprop="articleBody"]')[0]
    
article("https://www.birminghammail.co.uk/news/midlands-news/murder-accused-girlfriend-apologises-referring-22115654")